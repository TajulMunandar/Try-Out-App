...
class DashboardPenilaianController extends Controller
{
    public function index()
    {
        ...
        return view('dashboard.page.penilaian.index', [
            ...
            'paket_detail_id' => $firstPaketDetails->id
        ]);
    }
    ...
    public function filter(Request $request){
        ...
        if (auth()->user()->is_admin == 2 && isset($request->paket_detail_id)) {
            $penilaian = ViewScore::where('mahasiswa_prodi_id', $prodiId)
            ->where('paket_detail_id', $request->paket_detail_id)
            ->latest()
            ->get();
        } elseif (isset($request->paket_detail_id)) {
            $penilaian = ViewScore::where('paket_detail_id', $request->paket_detail_id)
            ->latest()
            ->get();
        }
